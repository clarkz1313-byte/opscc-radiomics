...
Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_15.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_15.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_15.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_15.csv"
TOP20_STRICT_CSV = OUT_DIR / "t2b_top20_strict_15.csv"
TOP20_EXT_CSV = OUT_DIR / "t2b_top20_ext_15.csv"
LOG_MD = OUT_DIR / "t2b_log_15.md"
...
SURV_GM_CONFIGS = [
    ("EST", "EST_120_sqrt", {"n_estimators": 120, "max_features": "sqrt", "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_200_sqrt", {"n_estimators": 200, "max_features": "sqrt", "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_320_sqrt", {"n_estimators": 320, "max_features": "sqrt", "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_200_half", {"n_estimators": 200, "max_features": 0.5, "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_320_half", {"n_estimators": 320, "max_features": 0.5, "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_200_all", {"n_estimators": 200, "max_features": 1.0, "min_samples_leaf": 1, "min_samples_split": 2}),
    ("EST", "EST_200_leaf2", {"n_estimators": 200, "max_features": "sqrt", "min_samples_leaf": 2, "min_samples_split": 4}),
    ("EST", "EST_320_leaf2", {"n_estimators": 320, "max_features": "sqrt", "min_samples_leaf": 2, "min_samples_split": 4}),
]
...
RESULT_COLUMNS = [
    ...
    "test_ci", "test_auc", "joint_test", "test_ba", "test_spe", "test_sen", "test_precision", "test_npv", "test_f1",
    "strict_score",
    "ext_ci", "ext_auc", "ext_ba", "ext_spe", "ext_sen", "ext_precision", "ext_npv", "ext_f1",
...
def _atomic_write_csv(df: pd.DataFrame, path: Path) -> None:
    ...
    raise PermissionError(
        f"Could not replace '{path.name}'. The target CSV is likely open in another program "
        "such as Excel, VS Code preview, or Explorer preview. Close the file and rerun."
    ) from last_err
...
def _make_surv(key: str, params: dict, n_est_override: int | None = None):
    if key == "EST":
        ...
        for opt in ["max_depth", "min_samples_split", "min_samples_leaf", "max_features"]:
            if opt in params:
                est_kwargs[opt] = params[opt]
        return ExtraSurvivalTrees(**est_kwargs)
    if key == "SVM":
...
def run(args: argparse.Namespace) -> None:
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v15 Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    if CHECKPOINTS_CSV.exists() and CHECKPOINTS_CSV.stat().st_size > 0:
        try:
            ...
            if confirm_df.empty and len(confirm_df.columns) == 0:
                confirm_df = _empty_confirm_df()
            elif not set(CONFIRM_COLUMNS).issubset(confirm_df.columns):
                print("Stage-2 checkpoint schema mismatch; restarting Stage 2 from saved Stage 1 screen.")
                confirm_df = _empty_confirm_df()
            else:
...
def main() -> None:
    ...
    print(
        f"ROOT={ROOT}\n"
        f"v15 final strict family sweep around 23652/27555 families | PT sizes={PT_SIZES} | CT sizes={CT_SIZES} | "
