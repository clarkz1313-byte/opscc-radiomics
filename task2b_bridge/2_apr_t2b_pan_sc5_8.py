"""
2_apr_t2b_pan_sc5_8.py  --  Task 2B v8

Lightweight local-neighborhood sweep around the strongest current 7ab anchor family:
- trial_no 5170 / 5279 / 5820 from t2b_all_results_7ab.csv
- same feature set: combo_id 6pt_2ct_02488
- same survival GM family anchor: FastSurvivalSVM

Purpose:
- spend laptop-friendly runtime on a targeted follow-up rather than another broad exhaustive search
- test whether ext_ci can be nudged upward by local survival-side tuning and tiny feature perturbations
- compare HPV heads only as downstream tradeoff readers, not as the main search axis

Neighborhood design:
- exact anchor feature set
- 1 PT swap (replace 1 of 6 PT with 1 unused PT) = 30 variants
- 1 CT swap (replace 1 of 2 CT with 1 unused CT) = 10 variants
- 5 PT + same 2 CT (drop 1 PT) = 6 variants
- 6 PT + 3 CT (add 1 CT) = 5 variants
Total feature neighborhoods = 52

GM design:
- Survival: SVM alpha grid + CoxPH baseline
- HPV: same 3 heads as v7/v7b
Approx total stage-1 pair evaluations = 52 x 5 x 3 = 780

Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_8.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_8.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_8.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_8.csv"
TOP20_JOINT_CSV = OUT_DIR / "t2b_top20_joint_8.csv"
TOP20_RFS_CSV = OUT_DIR / "t2b_top20_rfs_8.csv"
TOP20_HPV_CSV = OUT_DIR / "t2b_top20_hpv_8.csv"
LOG_MD = OUT_DIR / "t2b_log_8.md"
...
SURV_GM_CONFIGS = [
    ("SVM", "SVM_0003", {"alpha": 0.0003}),
    ("SVM", "SVM_001", {"alpha": 0.001}),
    ("SVM", "SVM_003", {"alpha": 0.003}),
    ("SVM", "SVM_01", {"alpha": 0.01}),
...
SCREEN_COLUMNS = [
    "feature_id", "origin", "n_total", "n_pt", "n_ct", "surv_gm", "hpv_gm", "pair_key",
    "oof_ci_s1", "oof_auc_s1", "joint_s1", "feat_pt", "feat_ct",
]

RESULT_COLUMNS = [
    "trial_no", "feature_id", "origin", "n_total", "n_pt", "n_ct", "surv_gm", "hpv_gm", "pair_key",
    "oof_ci_s1", "oof_auc_s1", "joint_s1", "feat_pt", "feat_ct",
...
def build_feature_sets():
    variants = []
    seen = set()
    def add_variant(origin: str, feat_pt: list[str], feat_ct: list[str]):
        key = (tuple(feat_pt), tuple(feat_ct))
        if key in seen:
            return
        seen.add(key)
        variants.append({"feature_id": f"fs_{len(variants):03d}", "origin": origin, "n_total": 1 + len(feat_pt) + len(feat_ct), "n_pt": len(feat_pt), "n_ct": len(feat_ct), "feat_pt": list(feat_pt), "feat_ct": list(feat_ct)})
    add_variant("anchor_exact", ANCHOR_PT, ANCHOR_CT)
    unused_pt = [f for f in UNION_PT if f not in ANCHOR_PT]
    unused_ct = [f for f in UNION_CT if f not in ANCHOR_CT]
    for i, removed in enumerate(ANCHOR_PT):
        for added in unused_pt:
            pt = list(ANCHOR_PT)
            pt[i] = added
            add_variant(f"pt_swap__out={removed}__in={added}", pt, ANCHOR_CT)
    for i, removed in enumerate(ANCHOR_CT):
        for added in unused_ct:
            ct = list(ANCHOR_CT)
            ct[i] = added
            add_variant(f"ct_swap__out={removed}__in={added}", ANCHOR_PT, ct)
    for i, removed in enumerate(ANCHOR_PT):
        pt = [feat for j, feat in enumerate(ANCHOR_PT) if j != i]
        add_variant(f"pt_drop__out={removed}", pt, ANCHOR_CT)
    for added in unused_ct:
        ct = list(ANCHOR_CT) + [added]
        add_variant(f"ct_add__in={added}", ANCHOR_PT, ct)
    return variants
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"v8 local sweep | feature neighborhoods={len(feature_sets):,} | total stage-1 pair evals={total_pair_evals:,}")
    if args.smoke:
        feature_sets = feature_sets[:6]
    ...
    for fs in feature_sets:
        ...
        for pair_key, (oof_ci, oof_auc) in pair_results.items():
            ...
            screen_rows.append({"feature_id": fs["feature_id"], "origin": fs["origin"], "n_total": fs["n_total"], "n_pt": fs["n_pt"], "n_ct": fs["n_ct"], "surv_gm": surv_gm, "hpv_gm": hpv_gm, "pair_key": pair_key, "oof_ci_s1": oof_ci, "oof_auc_s1": oof_auc, "joint_s1": ALPHA * oof_ci + (1.0 - ALPHA) * oof_auc, "feat_pt": "|".join(fs["feat_pt"]), "feat_ct": "|".join(fs["feat_ct"])} )
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v8 Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    summary_lines = ["# Task 2B v8 Log", "", "- Scope: local neighborhood sweep around 7ab anchor family 5170 / 5279 / 5820", f"- Runtime: {(time.time() - started_at)/60:.1f} min", f"- Feature neighborhoods evaluated: {len(feature_sets):,}", f"- Stage-1 soft-floor survivors: {len(passed_soft):,} / {len(screen_df):,}", f"- Stage-2 hard-floor survivors: {len(passed_hard):,} / {len(confirm_df):,}", f"- External evaluation rows: {len(result_df):,}", "", "## Best row by ext_ci", f"- trial_no={int(best_ci['trial_no'])} | feature_id={best_ci['feature_id']} | origin={best_ci['origin']} | {best_ci['surv_gm']} + {best_ci['hpv_gm']} | ext_ci={best_ci['ext_ci']:.3f} | ext_auc={best_ci['ext_auc']:.3f} | ext_ba={best_ci['ext_ba']:.3f} | ext_spe={best_ci['ext_spe']:.3f} | ext_sen={best_ci['ext_sen']:.3f}"]
    LOG_MD.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")
    print("\n" + "\n".join(summary_lines))
...
def main() -> None:
    args = parse_args()
    print(f"ROOT={ROOT}\nv8 local neighborhood sweep around 7ab anchor family")
    run(args)
