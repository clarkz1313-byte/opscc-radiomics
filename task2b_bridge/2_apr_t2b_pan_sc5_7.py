"""
2_apr_t2b_pan_sc5_7.py  --  Task 2B v7

Exhaustive combinatorial sweep over the union of RFS_consensus and GBC_HPV feature
pools (11 PT + 7 CT), at structures 4-5 PT x 2-3 CT, with all 9 GM pairs.

Total: 44,352 feature combinations x 9 GM pairs = 399,168 evaluations.
Estimated runtime: ~3.2 hours (based on v4 timing: 50.2 min / 104k evals).

Motivation: Optuna covered only ~1% of this space in v1/v2. With fold EPV 1.78-2.29,
OOF CI noise (~+/-0.05) is too large for the sampler to reliably rank candidates.
Exhaustive sweep removes sampler bias entirely, as proven by v3 which immediately
found candidates Optuna had missed on the same pool.

Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_7.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_7.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_7.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_7.csv"
TOP20_JOINT_CSV = OUT_DIR / "t2b_top20_joint_7.csv"
TOP20_RFS_CSV = OUT_DIR / "t2b_top20_rfs_7.csv"
TOP20_HPV_CSV = OUT_DIR / "t2b_top20_hpv_7.csv"
LOG_MD = OUT_DIR / "t2b_log_7.md"
...
def evaluate_on_ext(feat_pt, feat_ct, s_key, s_params, h_key, h_params, train_df, ext_df, n_boot):
    ...
    if n_boot > 0 and not np.isnan(ext_ci):
        ...
        for _ in range(n_boot):
            ...
            try:
                raw_auc = float(roc_auc_score(y_h_ext[idx], proba_eval[idx]))
...
def run(args: argparse.Namespace) -> None:
    ...
    print("\n--- Stage 1 exhaustive screen ---")
    if completed_rows >= expected_stage1_rows:
        print(f"Stage 1 checkpoint already complete: {completed_rows:,} / {expected_stage1_rows:,} rows. Skipping Stage 1.")
        ...
        for combo in combos:
            ...
            pair_results = _eval_combo_all_pairs(combo["feat_pt"], combo["feat_ct"], train_df, N_REPEATS_SCREEN, N_EST_SCREEN)
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v7 Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    summary_lines = [
        ...
        (
            ...
            f"ext_ba={best_joint['ext_ba']:.3f} ext_spe={best_joint['ext_spe']:.3f} ext_sen={best_joint['ext_sen']:.3f}"
        ),
        "",
        "## Top 5 by ext_ci",
    ]
...
def main() -> None:
    args = parse_args()
    print(f"ROOT={ROOT}\nv7 exhaustive union sweep | PT sizes={PT_SIZES} | CT sizes={CT_SIZES}")
    run(args)
