"""
2_apr_t2b_pan_sc5_11_ex496.py  --  Task 2B v11_ex496

Final a496-focused exhaustive local search.

Purpose:
- spend one last targeted search budget on the best current thesis-facing candidate family
- avoid broadening again across multiple anchors
- test whether a denser local search around a496 can approach ext_ci ~ 0.70 while preserving high specificity

Design:
- anchor: v8 trial 496 family
- exhaustive CT subset sweep over a focused nearby CT pool
- exhaustive 1-PT-swap neighborhoods around the a496 PT core
- dense local FastSurvivalSVM alpha grid plus CoxPH baseline
- same 3 HPV heads for direct comparability

Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_11_ex496.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_11_ex496.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_11_ex496.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_11_ex496.csv"
TOP20_JOINT_CSV = OUT_DIR / "t2b_top20_joint_11_ex496.csv"
TOP20_RFS_CSV = OUT_DIR / "t2b_top20_rfs_11_ex496.csv"
TOP20_HPV_CSV = OUT_DIR / "t2b_top20_hpv_11_ex496.csv"
LOG_MD = OUT_DIR / "t2b_log_11_ex496.md"
...
def build_feature_sets():
    ...
    for n_ct in [2, 3, 4]:
        for ct_feats in combinations(CT_POOL, n_ct):
            add_variant(f"ct_subset__nct={n_ct}", ANCHOR_PT, list(ct_feats))

    unused_pt = [f for f in UNION_PT if f not in ANCHOR_PT]
    for i, removed in enumerate(ANCHOR_PT):
        for added in unused_pt:
            pt_swapped = list(ANCHOR_PT)
            pt_swapped[i] = added
            for n_ct in [2, 3]:
                for ct_feats in combinations(CT_POOL, n_ct):
                    add_variant(f"pt_swap__out={removed}__in={added}__nct={n_ct}", pt_swapped, list(ct_feats))
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"v11_ex496 exhaustive local sweep | feature neighborhoods={len(feature_sets):,} | total stage-1 pair evals={total_pair_evals:,}")
    if args.smoke:
        feature_sets = feature_sets[:10]
    ...
    for fs in feature_sets:
        ...
        for pair_key, (oof_ci, oof_auc) in pair_results.items():
            ...
            screen_rows.append({
                "feature_id": fs["feature_id"], "origin": fs["origin"], "n_total": fs["n_total"], "n_pt": fs["n_pt"], "n_ct": fs["n_ct"],
                "surv_gm": surv_gm, "hpv_gm": hpv_gm, "pair_key": pair_key, "oof_ci_s1": oof_ci, "oof_auc_s1": oof_auc,
                "joint_s1": ALPHA * oof_ci + (1.0 - ALPHA) * oof_auc, "feat_pt": "|".join(fs["feat_pt"]), "feat_ct": "|".join(fs["feat_ct"]),
            })
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v11_ex496 Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    summary_lines = [
        "# Task 2B v11_ex496 Log",
        "",
        "- Scope: final exhaustive local search around the a496 family",
        ...
        "",
        "## Best row by ext_ci",
        f"- trial_no={int(best_ci['trial_no'])} | feature_id={best_ci['feature_id']} | origin={best_ci['origin']} | {best_ci['surv_gm']} + {best_ci['hpv_gm']} | ext_ci={best_ci['ext_ci']:.3f} | ext_auc={best_ci['ext_auc']:.3f} | ext_ba={best_ci['ext_ba']:.3f} | ext_spe={best_ci['ext_spe']:.3f} | ext_sen={best_ci['ext_sen']:.3f}",
    ]
    LOG_MD.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")
    print("\n" + "\n".join(summary_lines))
...
def main() -> None:
    args = parse_args()
    print(f"ROOT={ROOT}\nv11_ex496 exhaustive local sweep around a496")
    run(args)
