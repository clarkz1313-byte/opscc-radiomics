"""
2_apr_t2b_pan_sc5_9.py  --  Task 2B v9

Lightweight multi-anchor local sweep after v8.

Purpose:
- keep the search laptop-friendly
- prioritize high-specificity families while trying to push ext_ci toward 0.70
- focus on the best specificity-preserving near-miss anchors from v8 and 7ab

Main anchors:
- v8 trial 496 family: ext_ci ~ 0.681, ext_auc ~ 0.757, ext_spe = 1.000
- v8 fs_036 family: ext_ci ~ 0.681, ext_auc ~ 0.764, ext_spe ~ 0.571
- 7ab compact high-specificity family (trial 8920 / 9339 / 9343): ext_ci ~ 0.639, ext_spe >= 0.857
- 7ab broader family (trial 7921 / 8013): ext_ci ~ 0.639, ext_spe ~ 0.857

Design:
- exact anchors
- 1 PT swap neighbors
- 1 CT swap neighbors
- small add/drop variants around each anchor
- same 3 HPV heads as v7/v8
- survival side focused on local FastSurvivalSVM alpha tuning plus CoxPH baseline

Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_9.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_9.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_9.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_9.csv"
TOP20_JOINT_CSV = OUT_DIR / "t2b_top20_joint_9.csv"
TOP20_RFS_CSV = OUT_DIR / "t2b_top20_rfs_9.csv"
TOP20_HPV_CSV = OUT_DIR / "t2b_top20_hpv_9.csv"
LOG_MD = OUT_DIR / "t2b_log_9.md"
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"v9 multi-anchor local sweep | feature neighborhoods={len(feature_sets):,} | total stage-1 pair evals={total_pair_evals:,}")
    if args.smoke:
    ...
    for fs in feature_sets:
        ...
        for pair_key, (oof_ci, oof_auc) in pair_results.items():
            ...
            screen_rows.append({
                ...
                "oof_ci_s1": oof_ci, "oof_auc_s1": oof_auc, "joint_s1": ALPHA * oof_ci + (1.0 - ALPHA) * oof_auc,
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v9 Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    summary_lines = [
        "# Task 2B v9 Log",
        "",
        "- Scope: multi-anchor local sweep around high-specificity v8 and 7ab families",
...
def main() -> None:
    args = parse_args()
    print(f"ROOT={ROOT}\nv9 multi-anchor local sweep around high-specificity anchors")
    run(args)
