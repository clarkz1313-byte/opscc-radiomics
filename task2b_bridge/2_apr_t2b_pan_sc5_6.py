"""
2_apr_t2b_pan_sc5_6.py  --  Task 2B v6

Additional feature-pool sweep not covered by v5.

v5 already completed these pools:
  - RFS_consensus
  - UNIVAR_HPV
  - UNIVAR_JOINT
  - GBC_HPV

v6 covers the remaining pool families derived from the full Task 2B ranker
space, so a spare machine can extend pool coverage without duplicating v5.

Run a subset of pools with --pools (space-separated):
  python 2_apr_t2b_pan_sc5_6.py --pools WLOCO_JOINT_enon WLOCO_HPV_enon

GM pairs: same 9 as v4/v5 (3 survival x 3 HPV).
"""
...
def _rfs_loco_rank(train_df, feat_names, mode):
    ...
    for i, held in enumerate(centres):
        ...
        enon = float(ec * (nc - ec))
        if mode == "loco_evt":
            include = ec >= RFS_LOCO_EVENTS_MIN
            w, use_shrink = 1.0, False
        elif mode == "loco_epv_cut":
            include = ec >= RFS_LOCO_EVENTS_MIN and enon >= RFS_LOCO_ENON_MIN
            w, use_shrink = 1.0, False
        elif mode == "w_enon":
            include = True
            w, use_shrink = max(1.0, enon), True
        elif mode == "w_epv_cut":
            include = ec >= RFS_LOCO_EVENTS_MIN and enon >= RFS_LOCO_ENON_MIN
            w, use_shrink = max(1.0, enon), True
        else:
        ...
        for fi in range(n):
            try:
                ...
                ci = max(ci, 1.0 - ci)
                if use_shrink:
                    ci = (ec * ci + RFS_LOCO_KAPPA * 0.5) / (ec + RFS_LOCO_KAPPA)
                ci_mat[i, fi] = ci
            except Exception:
                ci_mat[i, fi] = 0.5
    valid = wts > 0
    if not np.any(valid): return np.full(n, 0.5)
    mat = np.where(np.isnan(ci_mat[valid]), 0.5, ci_mat[valid])
    if mode in {"w_enon", "w_epv_cut"}:
        return np.sum(mat * wts[valid][:, None], axis=0) / wts[valid].sum()
    return np.mean(mat, axis=0)


def _hpv_loco_rank(train_df, feat_names, mode):
    cids = train_df["CenterID"].to_numpy()
    y = train_df["HPV_binary"].to_numpy(int)
    x = train_df[feat_names].to_numpy(float)
    centres = np.unique(cids)
    n = len(feat_names)
    auc_mat = np.full((len(centres), n), np.nan)
    wts = np.zeros(len(centres))
    for i, held in enumerate(centres):
        mask = cids == held
        nc = int(mask.sum())
        ...
        for fi in range(n):
            try:
                auc = float(roc_auc_score(y_val, x[mask, fi]))
                auc = max(auc, 1.0 - auc)
                if use_shrink:
    ...
    mat = np.where(np.isnan(auc_mat[valid]), 0.5, auc_mat[valid])
    if mode in {"w_enon", "w_epv_cut"}:
        return np.sum(mat * wts[valid][:, None], axis=0) / wts[valid].sum()
    return np.mean(mat, axis=0)
...
def _gbc_hpv_rank(train_df, feat_names):
    ...
    try:
        ...
        gbc.fit(sc.transform(x), y, sample_weight=sample_weight)
...
def compute_pool(pool_name: str, train_df: pd.DataFrame) -> tuple[list[str], list[str]]:
    ...
    if pool_name == "UNIVAR_RFS":
        scores = {"UNIVAR_RFS": _univar_rfs(train_df, all_feats)}
        weights = [1.0]

    elif pool_name == "LOCO_RFS_evt":
        scores = {"LOCO_RFS_evt": _rfs_loco_rank(train_df, all_feats, "loco_evt")}
        weights = [1.0]

    elif pool_name == "LOCO_RFS_epv_cut":
        scores = {"LOCO_RFS_epv_cut": _rfs_loco_rank(train_df, all_feats, "loco_epv_cut")}
        weights = [1.0]

    elif pool_name == "WLOCO_RFS_enon":
        scores = {"WLOCO_RFS_enon": _rfs_loco_rank(train_df, all_feats, "w_enon")}
        weights = [1.0]

    elif pool_name == "WLOCO_RFS_epv_cut":
        scores = {"WLOCO_RFS_epv_cut": _rfs_loco_rank(train_df, all_feats, "w_epv_cut")}
        weights = [1.0]

    elif pool_name == "LOCO_HPV_evt":
        scores = {"LOCO_HPV_evt": _hpv_loco_rank(train_df, all_feats, "loco_evt")}
        weights = [1.0]

    elif pool_name == "LOCO_HPV_epv_cut":
        scores = {"LOCO_HPV_epv_cut": _hpv_loco_rank(train_df, all_feats, "loco_epv_cut")}
        weights = [1.0]

    elif pool_name == "WLOCO_HPV_enon":
        scores = {"WLOCO_HPV_enon": _hpv_loco_rank(train_df, all_feats, "w_enon")}
        weights = [1.0]

    elif pool_name == "WLOCO_HPV_epv_cut":
        scores = {"WLOCO_HPV_epv_cut": _hpv_loco_rank(train_df, all_feats, "w_epv_cut")}
        weights = [1.0]

    elif pool_name == "GBC_JOINT":
        rfs_sc = _univar_rfs(train_df, all_feats)
        gbc_sc = _gbc_hpv_rank(train_df, all_feats)
        scores = {"UNIVAR_RFS": rfs_sc, "GBC_HPV": gbc_sc}
        weights = [0.5, 0.5]

    elif pool_name == "WLOCO_JOINT_enon":
        rfs_sc = _rfs_loco_rank(train_df, all_feats, "w_enon")
        hpv_sc = _hpv_loco_rank(train_df, all_feats, "w_enon")
        scores = {"WLOCO_RFS_enon": rfs_sc, "WLOCO_HPV_enon": hpv_sc}
        weights = [0.5, 0.5]
...
def _eval_combo_single_pair(feat_pt, feat_ct, train_df, n_repeats, est_n, surv_label, hpv_label):
    """
    Evaluate only the requested GM pair for Stage 2 confirmation.
    This preserves the same CV logic while avoiding redundant 9-pair recomputation.
    """
    y_strat = (train_df["Relapse"].to_numpy(int) * 2
               + train_df["HPV_binary"].to_numpy(int)).clip(0, 3)
    rkf = RepeatedStratifiedKFold(n_splits=N_FOLDS, n_repeats=n_repeats, random_state=SEED)
...
def run_pool(pool_name: str, pt_pool: list[str], ct_pool: list[str],
    ...
    suffix = f"6_{pool_name}"
    screen_csv = OUT_DIR / f"t2b_screen_{suffix}.csv"
    all_csv    = OUT_DIR / f"t2b_all_results_{suffix}.csv"
    pair_keys = [f"{s[1]}|{h[1]}" for s, h in product(SURV_GM_CONFIGS, HPV_GM_CONFIGS)]
    screen_columns = [
        "pool", "combo_id", "n_total", "n_pt", "n_ct", "surv_gm", "hpv_gm", "pair_key",
    ...
    if screen_csv.exists() and screen_csv.stat().st_size > 0:
        ...
        missing_cols = [c for c in screen_columns if c not in screen_df.columns]
        if missing_cols:
            print(f"  [Pool={pool_name}] WARN: checkpoint missing columns {missing_cols}; restarting Stage 1.")
            screen_df = pd.DataFrame(columns=screen_columns)
        else:
            screen_df = screen_df[screen_columns]
        print(f"  [Pool={pool_name}] Loaded Stage 1 checkpoint: {len(screen_df):,} rows")
    else:
        screen_df = pd.DataFrame(columns=screen_columns)
    ...
    if not screen_df.empty:
        existing_pairs = {
            (int(row.combo_id), str(row.pair_key))
            for row in screen_df[["combo_id", "pair_key"]].itertuples(index=False)
        }
        combo_counts = screen_df.groupby("combo_id")["pair_key"].nunique()
        completed_combo_ids = {int(cid) for cid, n in combo_counts.items() if n == len(pair_keys)}

    screen_rows = screen_df.to_dict("records")
    t0 = time.time()
    for ci, (n_total, n_pt, n_ct, pt_f, ct_f) in enumerate(screen_combos):
        if ci in completed_combo_ids:
            continue
        ...
        for pair_key, (oof_ci, oof_auc) in pair_res.items():
            if (ci, pair_key) in existing_pairs:
                continue
    ...
    for _, row in top_screen.iterrows():
        ...
        oof_ci, oof_auc = _eval_combo_single_pair(
            feat_pt, feat_ct, train_df, N_REPEATS_CONFIRM, N_EST_CONFIRM, s_lbl, h_lbl
        )
        confirm_rows.append({
            **row.to_dict(),
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"\nv6: {len(pool_names)} pools × 9 GM pairs | N=[{N_MIN},{N_MAX}]")
    ...
    if ext_results:
        ...
        pd.DataFrame().to_csv(OUT_DIR / "t2b_top20_joint_6.csv", index=False)
        pd.DataFrame().to_csv(OUT_DIR / "t2b_top20_rfs_6.csv", index=False)
        log_lines += [
            "## Overall Summary",
...
def parse_args():
    ...
    p.add_argument("--pools", nargs="+", choices=ALL_POOL_NAMES,
                   help="Space-separated pool names (default: all additional v6 pools). "
                        "Example: --pools WLOCO_JOINT_enon WLOCO_HPV_enon")
    return p.parse_args()


def main():
    args = parse_args()
    print(f"ROOT={ROOT}\nv6: {len(ALL_POOL_NAMES)} additional pools x 9 GM pairs | N=[{N_MIN},{N_MAX}]")
    run(args)
