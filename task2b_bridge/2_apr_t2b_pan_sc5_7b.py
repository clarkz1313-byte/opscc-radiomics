"""
2_apr_t2b_pan_sc5_7b.py  --  Task 2B v7b

Gap-fill for v7: exhaustive sweep of the 6-PT tier only (4-5 PT already covered by v7).

Scope:
  - Same union pool as v7 (11 PT + 7 CT)
  - Structures: 6 PT x 2-3 CT ONLY  (the gap between v7's 4-5PT and the 70k ceiling)
  - All 9 GM pairs (identical to v4/v5/v6/v7)

Combos: C(11,6)*C(7,2) + C(11,6)*C(7,3) = 9702 + 16170 = 25,872 feature combos
         x 9 GM pairs = 232,848 evaluations
Estimated runtime: ~1.9 h (based on v4: 50.2 min / 104k evals)

Together v7 + v7b cover the full 4-6 PT x 2-3 CT exhaustive sweep (70,224 combos x9).

Run:
    python Apr_2026_task2B/2_apr_t2b_pan_sc5_7b.py
"""
...
SCREEN_CSV = OUT_DIR / "t2b_screen_7b.csv"
CHECKPOINTS_CSV = OUT_DIR / "t2b_checkpoints_7b.csv"
ALL_CSV = OUT_DIR / "t2b_all_results_7b.csv"
TOP20_JOINT_CSV = OUT_DIR / "t2b_top20_joint_7b.csv"
TOP20_RFS_CSV = OUT_DIR / "t2b_top20_rfs_7b.csv"
TOP20_HPV_CSV = OUT_DIR / "t2b_top20_hpv_7b.csv"
LOG_MD = OUT_DIR / "t2b_log_7b.md"

# ---------------------------------------------------------------------------
# Parameters  (aligned with v7)
# ---------------------------------------------------------------------------
SEED = 42
ALPHA = 0.60
N_FOLDS = 5
...
# v7b covers ONLY 6-PT structures — 4-5 PT are already done by v7
PT_SIZES = [6]
CT_SIZES = [2, 3]
...
# ---------------------------------------------------------------------------
# Core evaluation  (aligned with v7)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# External evaluation  (forked from v6 — includes ext_spe/ext_sen natively)
# ---------------------------------------------------------------------------
...
def load_data():
    ...
    print(
        f"Train n=87 (HPV-={(train_df['HPV_binary'] == 0).sum()}, Rel={train_df['Relapse'].sum()}) "
        f"| Ext n=27 (HPV-={(ext_df['HPV_binary'] == 0).sum()}, Rel={ext_df['Relapse'].sum()})"
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        f"v7b scope: 6-PT tier only | "
        f"Total feature combos: {total_combos:,} | Total stage-1 pair evals: {total_pair_evals:,}"
    )
    if args.smoke:
        combos = combos[:12]
        print(f"[SMOKE] limiting to first {len(combos)} feature combos")

    # Resume stage-1 checkpoint if present
    ...
    print("\n--- Stage 1 exhaustive screen (6-PT tier) ---")
    if completed_rows >= expected_stage1_rows:
        print(
            f"Stage 1 checkpoint already complete: "
            f"{completed_rows:,} / {expected_stage1_rows:,} rows. Skipping Stage 1."
        )
        screen_df = pd.DataFrame(screen_rows, columns=SCREEN_COLUMNS)
    else:
        for combo in combos:
            combo_id = combo["combo_id"]
            combo_done = {(cid, pk) for cid, pk in existing_pairs if cid == combo_id}
            if {pk for _, pk in combo_done} == expected_pair_keys:
                continue

            pair_results = _eval_combo_all_pairs(
                combo["feat_pt"], combo["feat_ct"], train_df, N_REPEATS_SCREEN, N_EST_SCREEN
            )
            for pair_key, (oof_ci, oof_auc) in pair_results.items():
                key = (combo_id, pair_key)
                ...
                screen_rows.append({
                    ...
                    "feat_pt": "|".join(combo["feat_pt"]),
                    "feat_ct": "|".join(combo["feat_ct"]),
                })
            ...
            if current_progress_bucket > last_progress_bucket:
                ...
                hit_rate = float(
                    ((temp_df["oof_ci_s1"] >= SOFT_CI_FLOOR) &
                     (temp_df["oof_auc_s1"] >= SOFT_AUC_FLOOR)).mean()
                )
                print(
    ...
    if passed_soft.empty:
        ...
        LOG_MD.write_text("# Task 2B v7b Log\n\nNo Stage-1 soft-floor survivors.\n", encoding="utf-8")
        return
    ...
    for _, row in passed_soft.iterrows():
        ...
        if current_confirm_progress_bucket > last_confirm_progress_bucket:
            ...
            hard_hit = float(
                ((temp_df["oof_ci"] >= HARD_CI_FLOOR) & (temp_df["oof_auc"] >= HARD_AUC_FLOOR)).mean()
            )
            print(
    ...
    if passed_hard.empty:
        ...
        summary = (
            "# Task 2B v7b Log\n\n"
            f"- Scope: 6-PT tier only (gap fill for v7)\n"
            ...
            "- No rows reached external evaluation.\n"
        )
    ...
    for idx, (_, row) in enumerate(passed_hard.iterrows(), start=1):
        ...
        ext = evaluate_on_ext(
            feat_pt, feat_ct, s_key, s_params, h_key, h_params, train_df, ext_df, N_BOOT
        )
    ...
    top_joint = result_df.sort_values(["joint_score", "ext_ci", "ext_auc"], ascending=False).head(20)
    top_rfs = result_df.sort_values(["ext_ci", "joint_score"], ascending=False).head(20)
    top_hpv = result_df.sort_values(["ext_auc", "joint_score"], ascending=False).head(20)
    top_joint.to_csv(TOP20_JOINT_CSV, index=False)
    top_rfs.to_csv(TOP20_RFS_CSV, index=False)
    top_hpv.to_csv(TOP20_HPV_CSV, index=False)

    dual = result_df[(result_df["ext_ci"] >= 0.60) & (result_df["ext_auc"] >= 0.70)]
    wall = time.time() - started_at
    best_joint = result_df.sort_values(["joint_score", "ext_ci", "ext_auc"], ascending=False).iloc[0]
    top5_ci = result_df.sort_values(["ext_ci", "joint_score"], ascending=False).head(5)
    top5_auc = result_df.sort_values(["ext_auc", "joint_score"], ascending=False).head(5)

    summary_lines = [
        "# Task 2B v7b Log",
        "",
        f"- Scope: 6-PT tier only (gap fill for v7's 4-5 PT)",
        f"- Started: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(started_at))}",
        f"- Finished: {time.strftime('%Y-%m-%d %H:%M:%S')}",
        f"- Runtime: {wall/60:.1f} min",
        f"- Total feature combos evaluated: {total_combos:,}",
        f"- Stage-1 soft-floor survivors: {len(passed_soft):,} / {len(screen_df):,} "
        f"({100*len(passed_soft)/max(len(screen_df),1):.2f}%)",
        f"- Stage-2 hard-floor survivors: {len(passed_hard):,} / {len(confirm_df):,} "
        f"({100*len(passed_hard)/max(len(confirm_df),1):.2f}%)",
        f"- External evaluation rows: {len(result_df):,}",
        f"- Dual-floor candidates (ext_ci >= 0.60 and ext_auc >= 0.70): {len(dual):,}",
        "",
        "## Best row by joint_score",
        (
            f"- trial_no={int(best_joint['trial_no'])} | combo_id={best_joint['combo_id']} | "
            f"{best_joint['surv_gm']} + {best_joint['hpv_gm']} | "
            f"N={int(best_joint['n_total'])} PT={int(best_joint['n_pt'])} CT={int(best_joint['n_ct'])} | "
            f"oof_ci={best_joint['oof_ci']:.3f} oof_auc={best_joint['oof_auc']:.3f} | "
            f"ext_ci={best_joint['ext_ci']:.3f} ext_auc={best_joint['ext_auc']:.3f} | "
            f"ext_ba={best_joint['ext_ba']:.3f} ext_spe={best_joint['ext_spe']:.3f} "
            f"ext_sen={best_joint['ext_sen']:.3f}"
        ),
        "",
        "## Top 5 by ext_ci",
    ]
    ...
    for _, row in top5_ci.iterrows():
        summary_lines.append(
            ...
            f"ext_ci={row['ext_ci']:.3f} ext_auc={row['ext_auc']:.3f} | "
            f"oof_ci={row['oof_ci']:.3f} oof_auc={row['oof_auc']:.3f}"
        )

    summary_lines += ["", "## Top 5 by ext_auc"]
    for _, row in top5_auc.iterrows():
        summary_lines.append(
            f"- trial_no={int(row['trial_no'])} | combo_id={row['combo_id']} | "
            f"{row['surv_gm']} + {row['hpv_gm']} | "
            f"ext_auc={row['ext_auc']:.3f} ext_ci={row['ext_ci']:.3f} | "
            f"oof_auc={row['oof_auc']:.3f} oof_ci={row['oof_ci']:.3f}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--smoke", action="store_true", help="Run on a small subset of combos.")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    print(f"ROOT={ROOT}\nv7b gap-fill: 6-PT tier | PT sizes={PT_SIZES} | CT sizes={CT_SIZES}")
    run(args)
