# -*- coding: utf-8 -*-
"""
Task 2 Stage 3: PT three-step feature-selection pipelines (primary continuation).
...
PT Stage 3 is a primary continuation: Stage 2 has strong signal across 22 pipelines.

Usage:
    cd "D:/Uppsala thesis"
    python Mar_2026_task2/14_mar_task2_stage3_PT.py
"""
...
OUT_CSV = RESULTS_DIR / "14_mar_task2_stage3_PT_result.csv"
OUT_PKL = RESULTS_DIR / "14_mar_task2_stage3_PT_features.pkl"
OUT_META = RESULTS_DIR / "14_mar_task2_stage3_PT_metadata.json"
CHECKPOINT = RESULTS_DIR / "14_mar_task2_stage3_PT_checkpoint.pkl"

BASELINE_AUC_S1 = 0.6917   # PT Stage 1 baseline (PT_1, C1 LASSO-30)
BASELINE_FEAT_S1 = 31
...
# ============================================================
# STAGE 2 SHORTLIST — hardcoded (22 pipelines, AUC2 >= 0.6400)
# Source: 13_mar_t2_fs_results/13_mar_task2_stage2_PT_result_processed.csv
# Processed CSV is a review artifact only; this list is authoritative.
# ============================================================

S2_SHORTLIST = [
    # s2_base_rank, category, s1_rank, s1_label, s1_key, s1_param,
    #   s2_method, s2_k, s2_label, auc2, fea2
    {
        "s2_base_rank": "PT_S2_1",
        "category": "C->D",
        "s1_rank": "PT_1",
        "s1_label": "C1: LASSO-Logistic (target 30)",
        "s1_key": "C1", "s1_param": 30,
        "s2_method": "D2", "s2_k": 20,
        "s2_label": "D2: XGBoost PermImp (top 20)",
        "auc2": 0.7494, "fea2": 20,
    },
    {
        "s2_base_rank": "PT_S2_3",
        "category": "B->C",
        "s1_rank": "PT_8",
    ...
    {
        "s2_base_rank": "PT_S2_4",
        "category": "C->D",
        "s1_rank": "PT_1",
        "s1_label": "C1: LASSO-Logistic (target 30)",
        "s1_key": "C1", "s1_param": 30,
        "s2_method": "D3", "s2_k": 30,
        "s2_label": "D3: GB PermImp (top 30)",
        "auc2": 0.7139, "fea2": 30,
    },
    {
        "s2_base_rank": "PT_S2_5",
        "category": "A->C",
        "s1_rank": "PT_18",
        "s1_label": "A7: ANOVA (top 200)",
        "s1_key": "A7", "s1_param": 200,
        "s2_method": "C2", "s2_k": 20,
        "s2_label": "C2: ElasticNet-Logistic (target 20)",
        "auc2": 0.6978, "fea2": 20,
    },
    {
        "s2_base_rank": "PT_S2_6",
        "category": "C->D",
        "s1_rank": "PT_1",
        "s1_label": "C1: LASSO-Logistic (target 30)",
        "s1_key": "C1", "s1_param": 30,
    ...
    {
        "s2_base_rank": "PT_S2_7",
        "category": "A->C",
        "s1_rank": "PT_17",
        "s1_label": "A0: MWU AUC (p<0.05 uncorr)",
        "s1_key": "A0", "s1_param": "uncorr",
        "s2_method": "C2", "s2_k": 30,
        "s2_label": "C2: ElasticNet-Logistic (target 30)",
        "auc2": 0.6944, "fea2": 30,
    },
    {
        "s2_base_rank": "PT_S2_8",
        "category": "A->C",
        "s1_rank": "PT_14",
    ...
    {
        "s2_base_rank": "PT_S2_17",
        "category": "C->D",
        "s1_rank": "PT_3",
        "s1_label": "C2: ElasticNet-Logistic (target 30)",
        "s1_key": "C2", "s1_param": 30,
        "s2_method": "D1", "s2_k": 20,
        "s2_label": "D1: RF PermImp (top 20)",
        "auc2": 0.6606, "fea2": 20,
    },
    {
        "s2_base_rank": "PT_S2_18",
        "category": "A->D",
        "s1_rank": "PT_14",
        "s1_label": "A7: ANOVA (top 100)",
        "s1_key": "A7", "s1_param": 100,
    ...
    {
        "s2_base_rank": "PT_S2_23",
        "category": "C->D",
        "s1_rank": "PT_2",
        "s1_label": "C2: ElasticNet-Logistic (target 60)",
        "s1_key": "C2", "s1_param": 60,
        "s2_method": "D2", "s2_k": 30,
        "s2_label": "D2: XGBoost PermImp (top 30)",
        "auc2": 0.6533, "fea2": 30,
    },
    {
        "s2_base_rank": "PT_S2_27",
        "category": "B->D",
        "s1_rank": "PT_23",
...
def selector_rf_perm(X_f: pd.DataFrame, y_f: np.ndarray, k: int) -> list[str]:
    ...
    imp = permutation_importance(clf, X_sub_val, y_sub_val, n_repeats=5, random_state=SEED, n_jobs=1)
...
def allowed_s3_methods(s2_base: dict) -> list[tuple[str, int]]:
    ...
    if cat in ("A->C", "B->C"):
        ...
        # S3: C or D (!=S2); A->D->C and B->D->C are allowed
        candidate_methods = s3_c_methods + s3_d_methods
    elif cat == "C->D":
...
print("=" * 80)
print(f"Task 2 Stage 3: {MODALITY} three-step pipelines (primary continuation)")
print("=" * 80)
...
for i, pipeline in enumerate(PIPELINES, start=1):
    ...
    # Skip C->D->D: S1 is a C-method feeding a D-method chain — extremely slow
    # (50-grid LASSO/EN × 5 folds) with low expected Stage 3 gain over Stage 2.
    if pipeline["s1_key"] in ("C1", "C2") and pipeline["pattern"].endswith("->D"):
        print(f"[{i:03d}/{len(PIPELINES)}] [SKIP_CDsD] {pid} {pipeline['pattern']} (C->D->D excluded)")
        done_ids.add(pid)
        continue
...
results_df.insert(0, "Rank", [f"PT_S3_{i}" for i in range(1, len(results_df) + 1)])
