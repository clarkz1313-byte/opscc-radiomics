"""
17_mar_task2_model_dev_INDIV.py

Task 2 Model Development — Script B (PC i9-12900K)
Individual radiomics rankers: UNIVAR, GBC, StabLASSO, BORDA

Reference plan: Mar_2026_task2/17_mar_task2_model_dev_plan.md (v5)

Design (GM + Coach exhaustive system, mirroring Task 1):
  - 4 INDIV rankers × 4 GMs × 1500 trials × 4 Coaches = 96,000 result rows
...
INDIV rankers (Task 1 analogs):
  UNIVAR     → directional AUC per feature: max(AUC, 1-AUC), all-train
               Applied separately to CT and PT pools (modality-split)
  GBC        → GradientBoostingClassifier feature importances, all-train
               Applied separately to CT and PT pools
  StabLASSO  → 50-bootstrap LASSO-logistic stability frequency, all-train
               Applied separately to CT and PT pools
  BORDA      → Borda count ensemble of UNIVAR + GBC + StabLASSO rank positions
               Applied separately to CT and PT pools

Run:
    cd "D:/Uppsala thesis" && python Mar_2026_task2/17_mar_task2_model_dev_INDIV.py
"""
...
ALL_CSV  = OUT_DIR / "17_mar_INDIV_all_results.csv"
CKPT_CSV = OUT_DIR / "17_mar_INDIV_checkpoint.csv"
TOP_CSV  = OUT_DIR / "17_mar_INDIV_top_results.csv"
PRED_CSV = OUT_DIR / "17_mar_INDIV_best_predictions.csv"
LOG_FILE = OUT_DIR / "17_mar_INDIV_log.txt"
...
# StabLASSO config
STAB_N_BOOT = 50                 # bootstrap resamples for stability freq
STAB_C      = 0.01               # L1 penalty C for stability LASSO

# GBC ranker config (mirrors Task 1's GBS ranker params)
GBC_N_EST   = 100
GBC_LR      = 0.1
GBC_DEPTH   = 3
GBC_SUBSAMP = 0.8

RANKER_NAMES = ["UNIVAR", "GBC", "StabLASSO", "BORDA"]
...
def _load_data():
    ...
    train_df = _merge(ct_train, pt_train)
    test_df  = _merge(ct_test,  pt_test)
    ext_df   = _merge(ct_ext,   pt_ext)
    return train_df, test_df, ext_df, ct_feat_list, pt_feat_list


# ============================================================
# INDIVIDUAL RANKERS (applied separately to CT and PT pools)
# ============================================================
...
def _univar_rank(X: np.ndarray, y: np.ndarray, feat_names: list[str]) -> list[str]:
    """
    UNIVAR: directional AUC per feature on all-train, max(AUC, 1-AUC).
    Mirrors Task 1's univar_rank() with safe_ci → roc_auc_score replacement.
    """
...
def _gbc_rank(X: np.ndarray, y: np.ndarray, feat_names: list[str]) -> list[str]:
    ...
    Mirrors Task 1's gbs_rank() with GBSurvival → GBClassifier.
    """
    try:
...
def _stab_lasso_rank(X: np.ndarray, y: np.ndarray, feat_names: list[str]) -> list[str]:
    """
    StabLASSO: bootstrap stability frequency of LASSO-logistic coefficients.
    50 bootstrap resamples; rank by fraction of resamples with |coef| > 0.
    Mirrors Task 1's stab_lasso_rank() with CoxnetSurvival → LogisticRegression(L1).
    """
    rng  = np.random.default_rng(SEED)
    freq = np.zeros(len(feat_names), dtype=float)
    ok   = 0
    for _ in range(STAB_N_BOOT):
        idx = rng.integers(0, len(X), size=len(X))
        for C in [STAB_C, STAB_C * 10, STAB_C * 100]:
            try:
                clf = LogisticRegression(
                    solver="saga", C=C, l1_ratio=1,
                    class_weight="balanced", random_state=SEED,
                    max_iter=3000,
                )
                clf.fit(X[idx], y[idx])
                freq += (np.abs(clf.coef_[0]) > 0).astype(float)
                ok += 1
                break
            except Exception:
                continue
    freq /= max(ok, 1)
    return [feat_names[i] for i in np.argsort(freq)[::-1]]


def _borda_combine(rank_lists: list[list[str]], feat_names: list[str]) -> list[str]:
    """
    BORDA: ensemble of rank lists via Borda count (mean rank position).
    Lower mean rank position = better feature. Ties broken alphabetically.
    Mirrors Task 1's borda_combine().
    """
    pos_map: dict[str, list[int]] = {f: [] for f in feat_names}
    for ranked in rank_lists:
        for rank, feat in enumerate(ranked):
            if feat in pos_map:
                pos_map[feat].append(rank)
    # Mean rank position per feature (lower = better ranked across methods)
    avg_pos = {f: (np.mean(pos_map[f]) if pos_map[f] else len(feat_names))
               for f in feat_names}
    return sorted(feat_names, key=lambda f: (avg_pos[f], f))
...
def _clinical_rank(X_clin: np.ndarray, y: np.ndarray,
    ...
    for C in np.logspace(-2, 1, 60):
        try:
            clf = LogisticRegression(solver="saga", C=C, l1_ratio=1,
                                     class_weight="balanced", random_state=SEED,
                                     max_iter=5000)
            clf.fit(Xs, y)
...
def _model_oof_auc(
    ...
    """
    5-fold OOF AUC for GM objective. Scalers on fold-train only.
...
def _coach_eval(
    ...
    Scalers fitted on X_train only and applied to both Test and Ext.
...
def _baseline_rows(ranker: str,
    ...
    configs = [
        ...
        ("Clin_only", clin_ranked[:2], CLINICAL_FEATURES,
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("17_mar_task2_model_dev_INDIV.py  —  PC i9-12900K")
    ...
    print(f"StabLASSO: {STAB_N_BOOT} bootstrap resamples, C={STAB_C}")
    ...
    # ----------------------------------------------------------
    # Pre-compute INDIV rankers (per pool: CT and PT separately)
    # ----------------------------------------------------------
    print("\n--- Pre-computing INDIV rankers ---")
    ...
    # GBC
    print("  Computing GBC (may take ~10s)...")
    ...
    # StabLASSO
    print(f"  Computing StabLASSO ({STAB_N_BOOT} bootstraps, may take ~30s)...")
    ct_stab = _stab_lasso_rank(X_ct_sc, y_train, ct_feat_list)
    pt_stab = _stab_lasso_rank(X_pt_sc, y_train, pt_feat_list)
    print(f"  StabLASSO: ct={len(ct_stab)}, pt={len(pt_stab)}")

    # BORDA (ensemble of UNIVAR + GBC + StabLASSO)
    ct_borda = _borda_combine([ct_univar, ct_gbc, ct_stab], ct_feat_list)
    pt_borda = _borda_combine([pt_univar, pt_gbc, pt_stab], pt_feat_list)
    print(f"  BORDA: ct={len(ct_borda)}, pt={len(pt_borda)}")

    rankings = {
        "UNIVAR":    {"ct": ct_univar, "pt": pt_univar, "clin": clin_ranked},
        "GBC":       {"ct": ct_gbc,    "pt": pt_gbc,    "clin": clin_ranked},
        "StabLASSO": {"ct": ct_stab,   "pt": pt_stab,   "clin": clin_ranked},
        "BORDA":     {"ct": ct_borda,  "pt": pt_borda,  "clin": clin_ranked},
    }
    ...
    for ranker in RANKER_NAMES:
        ...
        for gm in GM_NAMES:
            ...
            def objective(trial,
                          ...
                          _ct_r=ct_ranked, _pt_r=pt_ranked, _cl_r=clin_ranked_r,
                          _gm=gm):
    ...
    print("17_mar_task2_model_dev_INDIV.py finished.")
