# -*- coding: utf-8 -*-
"""
Task 2 Stage 1: CT Radiomics Feature Selection — HPV Classification (no-MDA A1 split)
=======================================================================================
...
Dev cohort: 87 HPV-known patients (CID1+CID6+CID2+CID7, no MDA)
  Input train: Mar_2026_task2/12_mar_task2_rad_data/13_mar_task2_CT_primary_train.csv
  Input test:  Mar_2026_task2/12_mar_task2_rad_data/13_mar_task2_CT_primary_test.csv
External: CHUS (27 HPV-known)
  Input: Mar_2026_task2/12_mar_task2_rad_data/12_mar_task2_CT_primary_ext.csv

Output: Mar_2026_task2/12_mar_task2_CT_stage1_result.csv
        Mar_2026_task2/12_mar_task2_CT_stage1_features.pkl  (for Stage 2 input)
        Mar_2026_task2/12_mar_task2_CT_stage1_metadata.json

Usage:
    cd "D:/Uppsala thesis"
    python Mar_2026_task2/12_mar_task2_stage1_CT.py
"""
...
SEED           = 42
MODALITY       = 'CT'
RESULTS_BASE   = '12_mar_task2_CT_stage1'
...
EXT_FILE = DATA_DIR / '12_mar_task2_CT_primary_ext.csv'
...
try:
    for k, label, key in [
        ...
        order    = np.argsort(imp.importances_mean)[::-1]
        selected = [X_train.columns[i] for i in order[:k]]
        # Nested CV — 200 estimators to keep runtime manageable across 5 folds
...
results_df.insert(0, 'Rank', [f'CT_{i}' for i in range(1, len(results_df) + 1)])
...
print(f'\n{"=" * 80}')
print(f'CT STAGE 1 COMPLETE')
print(f'{"=" * 80}')
print(f'  Next: 12_mar_task2_stage1_PT.py (parallel run on another machine)')
