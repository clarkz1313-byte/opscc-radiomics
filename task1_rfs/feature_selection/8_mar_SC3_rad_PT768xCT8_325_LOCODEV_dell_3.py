"""
8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3.py

Traditional SC3 with 4 LOCO-only rankers (no UNIVAR/BORDA/GBS/StabLASSO). No bootstrap.
Dell_3: N_MAX=12, CT_MIN=1 (No.244 has 1 CT), pre-filter UNIVAR top-30 for 2 rankers.

Rankers: LOCO_DEV, LOCO_evt, UNIVAR_top30_LOCO_std, UNIVAR_top30_LOCO_evt
Fix 1: RATIO_GRID_FULL = [0.0] + RATIO_GRID (n_clin=1 at ratio=0.0)
Fix 2: CLIN_RATIO_MAX = 0.20

Machine: Dell | TRIALS=1000 | N_MIN=6, N_MAX=12 | PT_MIN=4, CT_MIN=1
N_BOOT=0 (omit bootstrap to save time)

Outputs: Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3_outputs/

Usage:
  python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3.py
"""
...
OUTPUT_DIR = ROOT / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3_outputs"
...
N_MIN, N_MAX = 6, 12  # No.244 has N=6; cap at 12 to avoid inflated PT/CT
CLIN_RATIO_MIN, CLIN_RATIO_MAX = 0.05, 0.20
PT_MIN = 4
CT_MIN = 1  # No.244 has 1 CT; allow 4 PT + 1 CT
...
RANKING_METHODS = ["LOCO_DEV", "LOCO_evt", "UNIVAR_top30_LOCO_std", "UNIVAR_top30_LOCO_evt"]
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3_log.md"
...
def loco_dev_rank_events_threshold(X_full, y_full, center_ids, feat_names, min_events=10):
    """
    LOCO-DEV but only average over folds where held-out centre has >= min_events events.
    Excludes C1, C7, C8 when min_events=10 (low event counts).
    """
    unique_centres = np.unique(center_ids)
    events_per_centre = {}
    for c in unique_centres:
        mask = center_ids == c
        events_per_centre[c] = int(y_full["event"][mask].sum())
    eligible_centres = [c for c in unique_centres if events_per_centre[c] >= min_events]

    n_feats = X_full.shape[1]
    ci_matrix = np.full((len(eligible_centres), n_feats), np.nan)

    for fold_i, held_centre in enumerate(eligible_centres):
        mask_val = center_ids == held_centre
        mask_tr = ~mask_val
...
# ==========================================
print("=" * 70)
print("8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3")
print("4 LOCO rankers only; N_BOOT=0; N_MAX=12, CT_MIN=1")
print("Rankers: LOCO_DEV, LOCO_evt, UNIVAR_top30_LOCO_std, UNIVAR_top30_LOCO_evt")
...
rankings["LOCO_DEV"] = {
    "clin": clin_univar_ranked,
    "pt":   loco_pt_ranked,
    "ct":   loco_ct_ranked,
}
print("  LOCO_DEV done")

# LOCO_evt: events-threshold (min_events=10)
print("  Computing LOCO_evt (min_events=10)...")
loco_evt_pt, _ = loco_dev_rank_events_threshold(
    X_train_sc[:, pt_idx], y_train, dev_center_ids, pt_feat, min_events=10
)
loco_evt_ct, _ = loco_dev_rank_events_threshold(
    X_train_sc[:, ct_idx], y_train, dev_center_ids, ct_feat, min_events=10
)
rankings["LOCO_evt"] = {
    "clin": clin_univar_ranked,
    "pt":   loco_evt_pt,
...
# UNIVAR top-30 then LOCO (std and evt) on that pool
rad_feat = pt_feat + ct_feat
rad_idx = pt_idx + ct_idx
ranked_rad_univar = univar_rank(X_train_sc[:, rad_idx], y_train, rad_feat)[0]
top30_names = ranked_rad_univar[:30]
col_idx_30 = [feat_idx_map[f] for f in top30_names]
print("  UNIVAR top-30 rad pool:", len(top30_names), "features")

ranked_30_std, _ = loco_dev_rank(
    X_train_sc[:, col_idx_30], y_train, dev_center_ids, top30_names
)
pt_r_30_std = [f for f in ranked_30_std if f in pt_feat_set_idx]
ct_r_30_std = [f for f in ranked_30_std if f in ct_feat_set_idx]
rankings["UNIVAR_top30_LOCO_std"] = {
    "clin": clin_univar_ranked,
    "pt":   pt_r_30_std,
    "ct":   ct_r_30_std,
}
print("  UNIVAR_top30_LOCO_std done")

ranked_30_evt, _ = loco_dev_rank_events_threshold(
    X_train_sc[:, col_idx_30], y_train, dev_center_ids, top30_names, min_events=10
)
pt_r_30_evt = [f for f in ranked_30_evt if f in pt_feat_set_idx]
ct_r_30_evt = [f for f in ranked_30_evt if f in ct_feat_set_idx]
rankings["UNIVAR_top30_LOCO_evt"] = {
    "clin": clin_univar_ranked,
    "pt":   pt_r_30_evt,
    "ct":   ct_r_30_evt,
}
print("  UNIVAR_top30_LOCO_evt done")
...
print("  LOCO_DEV PT (top 15):", loco_pt_ranked[:15])
print("  LOCO_DEV CT (top 10):", loco_ct_ranked[:10])
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        # Seed No.244-like: N=6, n_clin=1, n_pt=4, n_ct=1
        if method in ("LOCO_DEV", "LOCO_evt", "UNIVAR_top30_LOCO_std", "UNIVAR_top30_LOCO_evt"):
            if gm_name in ("SVM", "CoxPH"):
                ...
                for alpha_val in ([0.001, 0.01, 0.1] if gm_name == "SVM" else [0.01, 0.1, 1.0]):
                    study.enqueue_trial({"total_n": 6, "clin_ratio": 0.167, "n_pt": 4, alpha_key: alpha_val})
                    study.enqueue_trial({"total_n": 8, "clin_ratio": 0.125, "n_pt": 5, alpha_key: alpha_val})
                    study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 6, alpha_key: alpha_val})
            elif gm_name == "GBS":
                study.enqueue_trial({"total_n": 6, "clin_ratio": 0.167, "n_pt": 4,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
                study.enqueue_trial({"total_n": 8, "clin_ratio": 0.125, "n_pt": 5,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            elif gm_name == "EST":
                study.enqueue_trial({"total_n": 6, "clin_ratio": 0.167, "n_pt": 4,
...
out_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3_all_results.csv"
...
top10_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_3 finished.")
