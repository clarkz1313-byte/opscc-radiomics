# -*- coding: utf-8 -*-
"""
PT Stage 5 PC run: PT_S3_32, 21, 25, 2000 trials (25: 400), WEIGHT_CV=0.60, FEA_MAX=30
"""
...
OUTPUT_DIR = SCRIPT_DIR / "1_mar_PT_Stage5_PC_outputs"
LOG_DIR = SCRIPT_DIR / "1_mar_PT_Stage5_PC_outputs"
CHECKPOINT_DIR = SCRIPT_DIR / "1_mar_PT_Stage5_PC_outputs"
...
SEED = 42
N_TRIALS = 2000
N_TRIALS_RANK_OVERRIDES = {"PT_S3_25": 400}
N_JOBS = 12
HEAVY_PIPELINE_N_JOBS = {
    "PT_S3_1": 1,    # D3_PermImp as S2
    "PT_S3_2": 1,    # D3_PermImp as S2
    "PT_S3_25": 1,   # D1_RSF as S2
    "PT_S3_32": 1,   # mRMR as S3 deadlock
}
...
def setup_logging() -> logging.Logger:
    ...
    log_file = LOG_DIR / f"1_mar_PT_Stage5_PC_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def objective_PT_S3_1(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        xn = trial.suggest_int("xgb_n_features", 8, 30)
...
def objective_PT_S3_2(trial: optuna.Trial) -> float:
    ...
    try:
        et = trial.suggest_int("elasticnet_target_features", 20, 100)
...
def objective_PT_S3_32(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        xn = trial.suggest_int("xgb_n_features", 10, 40)
        xne = trial.suggest_int("xgb_n_estimators", 80, 150)
        s2 = xgboost_survival_selection(
            X_train[s1], y_train, n_features=min(xn, len(s1)),
            n_estimators=xne, random_state=SEED
        ...
        mn = trial.suggest_int("mrmr_n_features", 8, 30)
...
STAGE5_SEEDS = {
    1: {"lasso_target_features": 74, "lasso_n_alphas": 90,
        "permimp_n_features": 30, "permimp_n_estimators": 337,
        "xgb_n_features": 36, "xgb_n_estimators": 119},
    2: {"elasticnet_target_features": 130, "elasticnet_l1_ratio": 0.821,
        "elasticnet_n_alphas": 124, "permimp_n_features": 64,
        "permimp_n_estimators": 358, "xgb_n_features": 31, "xgb_n_estimators": 111},
    21: {"cox_p_threshold": 0.0479, "xgb_n_features": 30, "xgb_n_estimators": 84,
         "lasso_target_features": 89, "lasso_n_alphas": 65},
    25: {"cox_p_threshold": 0.0207, "rsf_n_features": 42, "rsf_n_estimators": 329,
         "lasso_target_features": 119, "lasso_n_alphas": 116},
    32: {"cox_p_threshold": 0.0397, "xgb_n_features": 57, "xgb_n_estimators": 112,
         "mrmr_n_features": 20},
}
...
def run_optimization(rank: int, n_trials: int) -> optuna.Study:
    ...
    study_name = f"PT_S3_{rank}_Stage5_PC"
    ...
    if completed == 0:
        seed_params = STAGE5_SEEDS.get(rank)
        if seed_params:
            study.enqueue_trial(seed_params)
            logger.info("Enqueued Stage 4 seed for PT_S3_%s", rank)
...
def save_results(studies: Dict[int, optuna.Study]) -> None:
    ...
    for rank in PT_S3_RANKS:
        ...
        pf = OUTPUT_DIR / f"PT_S3_{rank}_Stage5_PC_trials_{run_date}.csv"
    ...
    out_file = OUTPUT_DIR / f"1_mar_PT_Stage5_PC_results_{run_date}.csv"
    ...
    for rank in PT_S3_RANKS:
        ...
        for t in sorted(study.trials, key=lambda x: x.number):
            ...
            row = {
                ...
                "intra_no": f"PT_S3_{rank}_Stage5_PC_{t.number}",
                "S3_rank": rank,
                "Pipeline": f"PT_S3_{rank}:{pipeline}",
    ...
    all_out = OUTPUT_DIR / f"1_mar_PT_Stage5_PC_ALLtrials_{run_date}.csv"
...
def main() -> None:
    ...
    print("\n" + "=" * 70)
    print("PT Stage 5 PC (2v): PT_S3_32, 21, 25 — 2000 trials (25: 400)")
    print("=" * 70 + "\n")
