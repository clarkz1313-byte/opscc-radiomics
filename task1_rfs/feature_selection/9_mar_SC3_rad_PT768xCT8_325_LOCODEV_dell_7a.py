"""
9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a.py

dell_7a: traditional confirmatory - 4 rankers (evt + enon_lam0 + epv_cut + loco_epv_cut).
Dell_4: N in [6,10], n_clin in [1,2], n_pt in [4,6], n_ct in [1,3].

Rankers:
  FULL_LOCO_evt
  FULL_WLOCO_enon (lambda=0)
  FULL_WLOCO_epv_cut
  FULL_LOCO_EPV_CUT
...
Outputs: Mar_2026/9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a_outputs/

Usage:
  python Mar_2026/9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a.py
"""
...
OUTPUT_DIR = ROOT / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a_outputs"
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a_log.md"
...
# ==========================================
print("=" * 70)
print("9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a")
print("Traditional SC3: 4-ranker confirmatory (evt + enon_lam0 + epv_cut + loco_epv_cut); N_BOOT=0")
print("Rankers: FULL_LOCO_evt / FULL_WLOCO_enon / FULL_WLOCO_epv_cut / FULL_LOCO_EPV_CUT")
print(f"N[{N_MIN},{N_MAX}] clin_ratio[{CLIN_RATIO_MIN:.2f},{CLIN_RATIO_MAX:.2f}] n_pt[{PT_MIN},{PT_MAX}] n_ct[{CT_MIN},{CT_MAX}] | TRIALS={TRIALS}")
...
# ==========================================
print("\n--- Pre-computing feature rankings (4 full-pool rankers) ---")
g_sc = StandardScaler()
...
def make_gm_objective(gm_name, clin_ridx, pt_ridx, ct_ridx):
    def objective(trial):
        ...
        trial.set_user_attr("clin_ratio", clin_ratio)
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        if gm_name in ("SVM", "CoxPH"):
            ...
            for alpha_val in [0.001, 0.01, 0.05, 0.1, 0.3]:
                ...
                study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 7, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 9, "clin_ratio": 0.111, "n_pt": 7, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 11, "clin_ratio": 0.091, "n_pt": 9, alpha_key: alpha_val})
        elif gm_name == "GBS":
            for s in [
                {"total_n": 6, "clin_ratio": 0.167, "n_pt": 4},
                {"total_n": 8, "clin_ratio": 0.125, "n_pt": 5},
                {"total_n": 10, "clin_ratio": 0.10, "n_pt": 6},
                {"total_n": 10, "clin_ratio": 0.10, "n_pt": 8},
                {"total_n": 10, "clin_ratio": 0.10, "n_pt": 7},
                {"total_n": 9, "clin_ratio": 0.111, "n_pt": 7},
                {"total_n": 11, "clin_ratio": 0.091, "n_pt": 9},
            ]:
                study.enqueue_trial({**s, "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
        elif gm_name == "EST":
            for s in [
                {"total_n": 6, "clin_ratio": 0.167, "n_pt": 4},
                {"total_n": 8, "clin_ratio": 0.125, "n_pt": 5},
                ...
                study.enqueue_trial({**s, "n_e": 300, "m_d": 12, "m_s_s": 6, "m_s_l": 4, "m_f": "sqrt"})
...
out_path = OUTPUT_DIR / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a_all_results.csv"
...
top10_path = OUTPUT_DIR / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n9_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_7a finished.")
