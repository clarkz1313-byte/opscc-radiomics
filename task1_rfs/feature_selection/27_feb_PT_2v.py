# -*- coding: utf-8 -*-
"""
Stage 1: PT Radiomics Feature Selection - CHUS+CHUP External Split (2v)
========================================================================
Single run version (no reproducibility loop).
Development cohort: 455 patients (CHUS, CHUP excluded; remaining centers)
External holdout: 90 patients (CHUS + CHUP)

Input:  Mar_2026/27_feb_PT_development.csv
Output: Mar_2026/27_feb_PT_2v_result.csv
        Mar_2026/27_feb_PT_2v_result_metadata.json
...
SEED = 42
RESULTS_BASE = '27_feb_PT_2v_result'
DATA_FILE = PROJECT_ROOT / 'Mar_2026' / '27_feb_PT_development.csv'
CHECKPOINT_RESULTS = OUTPUT_DIR / '27_feb_PT_2v_checkpoint_results.pkl'
CHECKPOINT_FEATURES = OUTPUT_DIR / '27_feb_PT_2v_checkpoint_features.pkl'

random.seed(SEED)
np.random.seed(SEED)

print("=" * 80)
print("PT STAGE 1: Feature Selection - CHUS+CHUP External Split 2v (Single Run)")
print("=" * 80)
...
print(f"\n[Step 1] Loading PT development data...")
...
if not is_completed(results, method_name):
    print(f"  Running {method_name}...")
    try:
        selected = elasticnet_cox_selection(X_train, y_train, l1_ratio=0.5, target_features=100, n_alphas=100, alpha_min_ratio=0.1)
        if len(selected) > 0:
...
results_df['Rank'] = [f'PT_{i}' for i in range(1, len(results_df) + 1)]
...
ax1.set_title('PT Stage 1 (CHUS+CHUP Split 2v): Top 10 Methods', fontsize=14, fontweight='bold')
...
ax2.set_title('PT Stage 1 (CHUS+CHUP Split 2v): Category Performance', fontsize=14, fontweight='bold')
...
ax3.set_title('PT Stage 1 (CHUS+CHUP Split 2v): Features vs Performance', fontsize=14, fontweight='bold')
ax3.legend()
...
ax4.set_title('PT Stage 1 (CHUS+CHUP Split 2v): Efficiency (Top 10)', fontsize=14, fontweight='bold')
