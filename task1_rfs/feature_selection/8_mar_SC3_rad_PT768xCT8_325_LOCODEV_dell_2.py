"""
8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2.py

Traditional SC3 with PT-forced modality split + LOCO_DEV as 5th ranker.
Dell_2 version — all fixes from dell + Fix 3: CT_MIN=2, n_ct up to 8.
...
Fix 3: CT_MIN=2, dense seeds at n_ct=4-8
  - Best both>=0.65 row had n_ct=6 (StabLASSO/SVM/EST, N=15, n_clin=1, n_pt=8, n_ct=6)
  - CT provides complementary signal to PT for CHUP

Machine: Dell overnight | TRIALS=1000 | N_MIN=6, N_MAX=20 | PT_MIN=4, CT_MIN=2
...
Outputs: Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2_outputs/

Usage:
  python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2.py
  SC3_TRIALS=800 python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2.py
"""
...
OUTPUT_DIR = ROOT / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2_outputs"
...
N_MIN, N_MAX = 6, 20
# Fix 2: CLIN_RATIO_MAX reduced to 0.20
CLIN_RATIO_MIN, CLIN_RATIO_MAX = 0.05, 0.20
PT_MIN = 4
# Fix 3: CT_MIN=2 (was 1), allows more CT features
CT_MIN = 2
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2_log.md"
...
# ==========================================
print("=" * 70)
print("8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2")
...
print("Fix 3: CT_MIN=2, dense seeds at n_ct=4-8 (best both>=0.65 had n_ct=6)")
print(f"PT_MIN={PT_MIN} | CT_MIN={CT_MIN} | N_MAX={N_MAX} | TRIALS={TRIALS}")
print("Rad pool: PT768 (35 PT feat) + CT8_325 (39 CT feat)")
print("Dev = full training | External: CHUS + CHUP | Machine: Dell overnight")
print("=" * 70)
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        # Fix 3: Dense seeds at n_ct=4-8, targeting best both>=0.65 anchor
        # Best row: StabLASSO/SVM/EST, N=15, n_clin=1, n_pt=8, n_ct=6
        # Explore: n_ct=4,5,6,7,8 with n_pt reduced accordingly
        if method == "StabLASSO":
            if gm_name in ("SVM", "CoxPH"):
                ...
                for alpha_val in ([0.001, 0.01, 0.1] if gm_name == "SVM" else [0.01, 0.1, 1.0]):
                    # Exact anchor: n_clin=1, n_pt=8, n_ct=6 (N=15)
                    study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8, alpha_key: alpha_val})
                    # Explore n_ct range with n_clin=1
                    study.enqueue_trial({"total_n": 14, "clin_ratio": 0.071, "n_pt": 10, alpha_key: alpha_val})  # n_ct=3
                    ...
                    study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 6,  alpha_key: alpha_val})  # n_ct=9
                    study.enqueue_trial({"total_n": 18, "clin_ratio": 0.056, "n_pt": 10, alpha_key: alpha_val})  # n_ct=7
            elif gm_name == "GBS":
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
                study.enqueue_trial({"total_n": 14, "clin_ratio": 0.071, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
                study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            elif gm_name == "EST":
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8,
                ...
                study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 8,
                                     "n_e": 300, "m_d": 12, "m_s_s": 6, "m_s_l": 4, "m_f": "sqrt"})
        elif method == "LOCO_DEV":
            if gm_name in ("SVM", "CoxPH"):
                alpha_key = "alpha_svm" if gm_name == "SVM" else "alpha_cox"
                for alpha_val in ([0.001, 0.01, 0.1] if gm_name == "SVM" else [0.01, 0.1, 1.0]):
                    study.enqueue_trial({"total_n": 14, "clin_ratio": 0.07, "n_pt": 8, alpha_key: alpha_val})
                    study.enqueue_trial({"total_n": 14, "clin_ratio": 0.07, "n_pt": 6, alpha_key: alpha_val})
                    study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 8, alpha_key: alpha_val})
                    study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 12, alpha_key: alpha_val})
            elif gm_name == "GBS":
                study.enqueue_trial({"total_n": 14, "clin_ratio": 0.07, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
                study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            elif gm_name == "EST":
            ...
            if gm_name in ("SVM", "CoxPH"):
                ...
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 14, "clin_ratio": 0.071, "n_pt": 8, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 16, "clin_ratio": 0.063, "n_pt": 8, alpha_key: alpha_val})
            elif gm_name == "GBS":
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            ...
            if gm_name in ("SVM", "CoxPH"):
                ...
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 12, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 11, "clin_ratio": 0.09, "n_pt": 6,  alpha_key: alpha_val})
            elif gm_name == "GBS":
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.067, "n_pt": 8,
                                     "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            elif gm_name == "EST":
...
out_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2_all_results.csv"
...
top10_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_2 finished.")
