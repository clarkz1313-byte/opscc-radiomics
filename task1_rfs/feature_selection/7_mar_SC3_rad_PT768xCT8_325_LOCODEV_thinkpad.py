"""
7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad.py

Traditional SC3 with PT-forced modality split, adding LOCO_DEV as a 5th ranker.

LOCO_DEV ranker (Leave-One-Centre-Out within Dev):
  - Train on 4 of 5 dev centres, test on held-out 5th.
  - Rank each PT feature by mean C-index across 5 folds.
  - Methodologically clean: only uses dev data, no external leakage.
  - PT817 feature ranks in LOCO_DEV: 1, 4, 7, 12, 21, 23, 24, 29, 31
    vs StabLASSO: 5, 6, 14, 20, 22, 27, 29, 30, 34
  - LOCO_DEV surfaces PT817 features at much shallower depths:
      n_pt >= 12 covers 4/9 PT817 features
      n_pt >= 24 covers 7/9 PT817 features

Seed enqueues for LOCO_DEV ranker:
  - Target n_pt=12 (4/9 PT817), n_pt=18 (6-7/9 PT817), n_pt=22 (7/9 PT817)
  - Combined with SVM/CoxPH alpha values that worked for PT817-type depth

Machine: Thinkpad | TRIALS=1000 | N_MIN=6, N_MAX=20 | PT_MIN=4, CT_MIN=1
...
Outputs: Mar_2026/7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_outputs/

Usage:
  python Mar_2026/7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad.py
  SC3_TRIALS=800 python Mar_2026/7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad.py
"""
...
OUTPUT_DIR = ROOT / "7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_outputs"
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_log.md"
...
def loco_dev_rank(X_full, y_full, center_ids, feat_names):
    ...
    for fold_i, held_centre in enumerate(unique_centres):
        ...
        for fi in range(n_feats):
            # Use univariate score directly on the held-out centre
            # (no model needed: just rank by the raw feature value)
...
# ==========================================
print("=" * 70)
print("7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad")
...
# Clinical LOCO_DEV: same univar approach within dev, fall back to UNIVAR for clin
...
# PT817 CT features are rank 37/39 and 38/39 in the static CT8_325 pool.
# LOCO_DEV may surface them much higher — critical for CT side of replication.
CT817_FEATURES = {
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        #   n_pt>=12 covers 4/9, n_pt>=24 covers 7/9, n_pt>=31 covers 8/9
        ...
        # BORDA: intermediate between UNIVAR and StabLASSO
        if method == "LOCO_DEV":
            # Primary target: LOCO_DEV surfaces PT817 at much shallower depths
            # n_pt=12 gets 4/9, n_pt=22 gets 7/9 PT817 features
...
out_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_all_results.csv"
...
top10_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n7_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad finished.")
