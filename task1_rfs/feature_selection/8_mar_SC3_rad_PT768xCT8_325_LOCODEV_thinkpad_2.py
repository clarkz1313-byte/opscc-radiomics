"""
8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2.py

Traditional SC3 with PT-forced modality split + LOCO_DEV as 5th ranker.
Thinkpad_2 version — targeted narrow search around the best-so-far anchor.

Best anchor (StabLASSO/SVM/EST, No.676 equivalent):
  ranker=StabLASSO | GM=SVM or EST | N=15 | n_clin=1 | n_pt=8 | n_ct=6
  CHUS=0.657, CHUP=0.662 (best both>=0.65 found)

Strategy: Dense Optuna seeding around anchor, explore n_pt=6-10, n_ct=4-9,
all at n_clin=1. Cover both SVM and EST GMs since they both showed
reasonable CHUS/CHUP near anchor. CT_MIN=2 to guarantee CT contribution.

Fix 1: RATIO_GRID_FULL = [0.0] + RATIO_GRID (n_clin=1 guaranteed in Stage 2)
Fix 2: CLIN_RATIO_MAX = 0.20
Fix 3: CT_MIN=2

Machine: Thinkpad | TRIALS=1200 | N_MIN=6, N_MAX=20 | PT_MIN=4, CT_MIN=2
...
Config: N_FOLDS=5, SEED=42, TRIALS=1200

Outputs: Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2_outputs/

Usage:
  python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2.py
  SC3_TRIALS=800 python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2.py
"""
...
OUTPUT_DIR = ROOT / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2_outputs"
...
# Fix 3: CT_MIN=2
CT_MIN = 2
TRIALS = int(os.getenv("SC3_TRIALS", "1200"))
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2_log.md"
...
# ==========================================
# 3. STAGE 1: GM OPTUNA SEARCH — targeted anchor seeding
# ==========================================
print(f"\n--- Stage 1: Optuna GM search ({TRIALS} trials each) ---")
print("Targeted dense seeding around StabLASSO/SVM/EST anchor (n_clin=1, n_pt=8, n_ct=6)")
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        # Dense targeted seeding: anchor is n_clin=1, n_pt=8, n_ct=6 (N=15)
        # Sweep: n_pt in [6,7,8,9,10], n_ct in [4,5,6,7,8,9] at n_clin=1
        # Focus most seeds on StabLASSO (where anchor was found); fewer for others
        if gm_name in ("SVM", "CoxPH"):
            ...
            if method == "StabLASSO":
                # Most seeds here — this is where anchor was found
                for alpha_val in alpha_vals:
                    for n_pt_seed, n_ct_seed in [(8,6),(7,6),(9,6),(8,5),(8,7),(6,6),(10,6),(8,4),(8,8)]:
                        N_seed = 1 + n_pt_seed + n_ct_seed
                        if N_seed > N_MAX:
                            continue
                        study.enqueue_trial({"total_n": N_seed, "clin_ratio": 1.0/N_seed,
                                             "n_pt": n_pt_seed, alpha_key: alpha_val})
            else:
                for alpha_val in alpha_vals[:3]:
                    for n_pt_seed, n_ct_seed in [(8,6),(7,6),(9,6),(8,5),(8,7)]:
                        N_seed = 1 + n_pt_seed + n_ct_seed
                        if N_seed > N_MAX:
                            continue
                        study.enqueue_trial({"total_n": N_seed, "clin_ratio": 1.0/N_seed,
            ...
            est_params_anchor = {"n_e": 300, "m_d": 12, "m_s_s": 6, "m_s_l": 4, "m_f": "sqrt"}
            if method == "StabLASSO":
                for n_pt_seed, n_ct_seed in [(8,6),(7,6),(9,6),(8,5),(8,7),(6,6),(10,6),(8,4),(8,8),(8,9)]:
                    N_seed = 1 + n_pt_seed + n_ct_seed
                    if N_seed > N_MAX:
                        continue
                    study.enqueue_trial({"total_n": N_seed, "clin_ratio": 1.0/N_seed,
                                         "n_pt": n_pt_seed, **est_params_anchor})
                # Also try n_clin=2 variants
                for n_pt_seed, n_ct_seed in [(8,6),(8,5)]:
                    N_seed = 2 + n_pt_seed + n_ct_seed
                    if N_seed > N_MAX:
                        continue
                    study.enqueue_trial({"total_n": N_seed, "clin_ratio": 2.0/N_seed,
                                         "n_pt": n_pt_seed, **est_params_anchor})
            else:
                for n_pt_seed, n_ct_seed in [(8,6),(7,6),(9,6),(8,5),(8,7)]:
                    N_seed = 1 + n_pt_seed + n_ct_seed
            ...
            gbs_params_anchor = {"n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8}
            for n_pt_seed, n_ct_seed in [(8,6),(7,6),(9,6),(8,5),(8,7)]:
                N_seed = 1 + n_pt_seed + n_ct_seed
                if N_seed > N_MAX:
                    continue
                study.enqueue_trial({"total_n": N_seed, "clin_ratio": 1.0/N_seed,
                                     "n_pt": n_pt_seed, **gbs_params_anchor})
...
out_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2_all_results.csv"
...
top10_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print(f"\nDual ok (both>=0.70): {((scan_df['ci_chus']>=0.70) & (scan_df['ci_chup']>=0.70)).sum()}")
print(f"Dual ok (both>=0.65): {((scan_df['ci_chus']>=0.65) & (scan_df['ci_chup']>=0.65)).sum()}")
print(f"Max CHUS: {scan_df['ci_chus'].max():.4f}")
print(f"Max CHUP: {scan_df['ci_chup'].max():.4f}")

print("\n8_mar_SC3_rad_PT768xCT8_325_LOCODEV_thinkpad_2 finished.")
