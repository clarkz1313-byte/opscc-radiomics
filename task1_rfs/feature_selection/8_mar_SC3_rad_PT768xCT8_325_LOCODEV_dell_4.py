"""
8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4.py

Traditional SC3 with UNIVAR30 + 5 LOCO variants. No bootstrap.
Dell_4: N in [6,10], n_clin in [1,2], n_pt in [4,6], n_ct in [1,3].

Rankers:
  UNIVAR30_LOCO_evt
  UNIVAR30_WLOCO_epv_ratio
  UNIVAR30_WLOCO_enon
  UNIVAR30_WLOCO_epv_cut
  UNIVAR30_WLOCO_epv_x_enon
Fix: RATIO_GRID_FULL = [0.0] + RATIO_GRID (n_clin=1 at ratio=0.0)

Machine: Dell | TRIALS=1000 | N_MIN=6, N_MAX=10 | PT_MIN=4, CT_MIN=1
N_BOOT=0 (omit bootstrap to save time)

Outputs: Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4_outputs/

Usage:
  python Mar_2026/8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4.py
"""
...
OUTPUT_DIR = ROOT / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4_outputs"
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4_log.md"
...
# ==========================================
print("=" * 70)
print("8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4")
print("Traditional SC3: UNIVAR30 + 5 LOCO variants; N_BOOT=0")
print("Rankers: UNIVAR30_LOCO_evt / WLOCO_epv_ratio / WLOCO_enon / WLOCO_epv_cut / WLOCO_epv_x_enon")
...
# ==========================================
# 2. PRE-COMPUTE RANKINGS — UNIVAR30 + 5 LOCO variants
# ==========================================
print("\n--- Pre-computing feature rankings (UNIVAR30 + 5 LOCO variants) ---")
g_sc = StandardScaler()
...
ranker_mode_map = {
    "UNIVAR30_LOCO_evt": "loco_evt",
    "UNIVAR30_WLOCO_epv_ratio": "w_epv_ratio",
    "UNIVAR30_WLOCO_enon": "w_enon",
    "UNIVAR30_WLOCO_epv_cut": "w_epv_cut",
    "UNIVAR30_WLOCO_epv_x_enon": "w_epv_x_enon",
}
for rk in RANKING_METHODS:
    ranked_rk, _ = loco_rank_mode(
        X_train_sc[:, univar30_idx],
        y_train,
        dev_center_ids,
        univar30,
    ...
    print(f"  {rk} done (pt={len(pt_r)}, ct={len(ct_r)})")
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        if gm_name in ("SVM", "CoxPH"):
            ...
            for alpha_val in ([0.001, 0.01, 0.1] if gm_name == "SVM" else [0.01, 0.1, 1.0]):
                study.enqueue_trial({"total_n": 6, "n_clin": 1, "n_pt": 4, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 8, "n_clin": 1, "n_pt": 5, alpha_key: alpha_val})
                study.enqueue_trial({"total_n": 10, "n_clin": 2, "n_pt": 6, alpha_key: alpha_val})
        elif gm_name == "GBS":
            study.enqueue_trial({"total_n": 6, "n_clin": 1, "n_pt": 4,
                                 "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
            study.enqueue_trial({"total_n": 8, "n_clin": 1, "n_pt": 5,
                                 "n_e": 100, "lr": 0.1, "m_d": 3, "ss": 0.8})
        elif gm_name == "EST":
            study.enqueue_trial({"total_n": 6, "n_clin": 1, "n_pt": 4,
...
out_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4_all_results.csv"
...
top10_path = OUTPUT_DIR / "8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n8_mar_SC3_rad_PT768xCT8_325_LOCODEV_dell_4 finished.")
