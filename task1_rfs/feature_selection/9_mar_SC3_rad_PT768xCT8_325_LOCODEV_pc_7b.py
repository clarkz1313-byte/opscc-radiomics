"""
9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b.py

Exhaustive SC3 replication-first run.
pc_7b: soft-weight spectrum - FULL_WLOCO_enon (lambda=0) + FULL_LOCO_EPV_CUT baseline.
...
Expected rows: TRIALS * 2 * 2 * 4 (e.g. 2000*16 = 32k)

Outputs: Mar_2026/9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b_outputs/

Usage:
  python Mar_2026/9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b.py
  SC3_TRIALS=1000 python Mar_2026/9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b.py
"""
...
OUTPUT_DIR = ROOT / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b_outputs"
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b_log.md"
...
def enqueue_structural_seeds(study, alpha_key):
    ...
    for s in structures:
        n_clin = max(1, round(s["total_n"] * s["clin_ratio"]))
        n_rad = s["total_n"] - n_clin
        n_ct = n_rad - s["n_pt"]
...
def enqueue_phase_b_focus(study, alpha_key, n_enqueue, seed=SEED):
    ...
    cr_vals = [float(t.params.get("clin_ratio", CLIN_RATIO_MIN)) for t in top]
    ...
    cr_low, cr_high = float(np.percentile(cr_vals, 20)), float(np.percentile(cr_vals, 80))
    ...
    for _ in range(n_enqueue):
        ...
        clin_ratio = float(rng.uniform(max(CLIN_RATIO_MIN, cr_low), min(CLIN_RATIO_MAX, cr_high)))
        ...
        p = {"total_n": N, "clin_ratio": clin_ratio, "n_pt": n_pt, alpha_key: float(np.exp(loga))}
        study.enqueue_trial(p)
...
# ==========================================
print("=" * 70)
print("9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b")
print("Exhaustive: pc_7b soft-weight spectrum (WLOCO_enon lam0 + LOCO_EPV_CUT) | N_BOOT=0")
print("GM: SVM+CoxPH | Coaches: EST/SVM/GBS/CoxPH")
print(f"N in [{N_MIN},{N_MAX}] | clin_ratio in [{CLIN_RATIO_MIN:.2f},{CLIN_RATIO_MAX:.2f}] | "
...
# ==========================================
# 2. PRE-COMPUTE RANKINGS — FULL ranker only
# ==========================================
print("\n--- Pre-computing feature rankings (FULL_WLOCO_enon + FULL_LOCO_EPV_CUT) ---")
g_sc = StandardScaler()
...
out_path = OUTPUT_DIR / "9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b_all_results.csv"
...
print("\n9_mar_SC3_rad_PT768xCT8_325_LOCODEV_pc_7b finished.")
