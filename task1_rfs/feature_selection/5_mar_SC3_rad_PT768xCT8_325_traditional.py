"""
5_mar_SC3_rad_PT768xCT8_325_traditional.py

Traditional SC3 (same structure as 4_mar_SC3_rad_PT817xCT8_325_final.py) but
using PT_S3_1_768 (Stage4 best CV4, 35 features) instead of PT817 (9 features).

Purpose: Option A bias-check experiment.
  - CT used Stage4 best-CV4 (CT_S3_8_235, 39 feat) directly. No Stage5 for CT.
  - PT817 came from Stage5 trio-skimming — introduces selection bias concern.
  - PT_S3_1_768 = Stage4 inter_no=1, best CV4=0.7671, S3_rank=1 pipeline
    (C1_LASSO -> D3_PermImp_50 -> D2_XGBoost_50), 35 features.
  - ALL 9 PT817 features are confirmed subsets of these 35 features (9/9 overlap).
  - If SC3 finds similar CHUS/CHUP with this 35-feature pool, it proves Stage5
    did not introduce selection bias: SC3 Optuna would have found the same signal.

Differences from 4_mar_SC3_rad_PT817xCT8_325_final.py:
  1. PT feature file: PT_inter1_768_features.csv (35 feat) instead of PT_inter817 (9 feat)
  2. Output dir: 5_mar_SC3_rad_PT768xCT8_325_traditional_outputs/
  3. TRIALS default: 1000 (run on Dell for fast results)
  4. Seed enqueue: same as _final (BORDA/SVM near No244 region)

Rad pool: PT_S3_1_768 (35 PT feat) + CT8_325 (39 CT feat) = 74 rad features
Clinical: 3 features (Age, Gender_Male, Treatment_CRT)
Config: N_FOLDS=5, SEED=42, TRIALS=1000, N_MIN=5, N_MAX=25

Outputs: Mar_2026/5_mar_SC3_rad_PT768xCT8_325_traditional_outputs/

Usage:
  python Mar_2026/5_mar_SC3_rad_PT768xCT8_325_traditional.py
  SC3_TRIALS=700 python Mar_2026/5_mar_SC3_rad_PT768xCT8_325_traditional.py
"""
...
OUTPUT_DIR = ROOT / "5_mar_SC3_rad_PT768xCT8_325_traditional_outputs"
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "5_mar_SC3_rad_PT768xCT8_325_traditional_log.md"
...
# ==========================================
print("=" * 70)
print("5_mar_SC3_rad_PT768xCT8_325_traditional")
print("Bias-check: PT_S3_1_768 (Stage4 best CV4, 35 feat) x CT8_325 (39 feat)")
print("All 9 PT817 features confirmed subset of PT_S3_1_768 (9/9 overlap)")
print("Dev = full training | External: CHUS + CHUP")
print("=" * 70)
...
# ==========================================
# 3. STAGE 1: GM OPTUNA SEARCH
# ==========================================
print(f"\n--- Stage 1: Optuna GM search ({TRIALS} trials each) ---")
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        # Seed enqueue: warm-start TPE near known good region (No244: N=6, 1 clin, small alpha).
        ...
        stage1_results[(method, gm_name)] = {
            ...
            "clin_r": clin_r,
            "rad_r": rad_r,
            "anchor_features": bt.user_attrs["features"],
        }
        print(f"N={bt.user_attrs['N']} | OOF={bt.user_attrs['oof_ci']:.4f}")

# ==========================================
# 4. STAGE 2: SUBSTITUTION SCAN (fixed — reset per ratio)
# ==========================================
print("\n--- Stage 2: Substitution scan across clin_ratio grid ---")
...
for method in RANKING_METHODS:
    for gm_name in GM_MODEL_NAMES:
        ...
        for ratio in RATIO_GRID:
            # Reset from anchor at every ratio (bug-fixed version)
            current_clin_feats = [f for f in anchor_feats if f in clin_feat]
            current_rad_feats = [f for f in anchor_feats if f in rad_feat]

            target_nc = min(max(1, round(N_star * ratio)), len(clin_r), N_star - 1)
            temp_model_fn = lambda: build_model_fn(gm_name, gm_params)

            while len(current_clin_feats) < target_nc:
                if len(current_rad_feats) == 0:
                    break
                next_clin = next_available_feature(clin_r, current_clin_feats)
                if next_clin is None:
                    break
                current_clin_feats.append(next_clin)
                lineup_before_drop = current_clin_feats + current_rad_feats
                col_idx_temp = [feat_idx_map[f] for f in lineup_before_drop]
                X_temp_sc = StandardScaler().fit_transform(X_train[:, col_idx_temp])
                temp_model = temp_model_fn().fit(X_temp_sc, y_train)
                lvi_rad = get_least_important_feature(
                    X_temp_sc, y_train, temp_model, current_rad_feats, lineup_before_drop
                )
                current_rad_feats.remove(lvi_rad)
            ...
            while len(current_clin_feats) > target_nc:
                ...
                X_temp_sc = StandardScaler().fit_transform(X_train[:, col_idx_temp])
                temp_model = temp_model_fn().fit(X_temp_sc, y_train)
                lvi_clin = get_least_important_feature(
                    X_temp_sc, y_train, temp_model, current_clin_feats, lineup_before_drop
                )
                current_clin_feats.remove(lvi_clin)
                next_rad = next_available_feature(rad_r, current_rad_feats)
                if next_rad is not None:
                    current_rad_feats.append(next_rad)

            feat_names = current_clin_feats + current_rad_feats
            ...
            for coach_name in COACH_NAMES:
                ...
                row = {
                    ...
                    "rad_names": "; ".join(current_rad_feats),
                }
                all_scan_rows.append(row)
                print(
                    f"  {method}/{gm_name}/{coach_name} | R={ratio:.2f} "
                    f"| N={len(feat_names)} ({len(current_clin_feats)}c+{len(current_rad_feats)}r) "
...
out_path = OUTPUT_DIR / "5_mar_SC3_rad_PT768xCT8_325_traditional_all_results.csv"
...
top10 = scan_df.nlargest(10, "trio_mean")[
    ["No", "ranker", "gm_model", "coach_model", "N", "n_clin", "n_rad",
     "oof_ci", "ci_chus", "ci_chup", "trio_mean", "trio_gap",
     "boot_ci_chus_lo", "boot_ci_chus_hi", "boot_ci_chup_lo", "boot_ci_chup_hi",
     "clin_names", "rad_names"]
]
print("\nTop 10 by Trio mean (OOF + CHUS + CHUP):")
print(top10.to_string(index=False))

top10_path = OUTPUT_DIR / "5_mar_SC3_rad_PT768xCT8_325_traditional_top10.csv"
top10.to_csv(top10_path, index=False)
print(f"\nSaved top10: {top10_path.name}")

print("\n5_mar_SC3_rad_PT768xCT8_325_traditional finished.")
