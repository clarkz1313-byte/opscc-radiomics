"""
7_mar_SC3_rad_PT768xCT8_325_deepPT_pc.py
...
Outputs: Mar_2026/7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_outputs/

Usage:
  python Mar_2026/7_mar_SC3_rad_PT768xCT8_325_deepPT_pc.py
  SC3_TRIALS=1500 python Mar_2026/7_mar_SC3_rad_PT768xCT8_325_deepPT_pc.py
"""
...
OUTPUT_DIR = ROOT / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_outputs"
...
N_MIN, N_MAX = 6, 20                 # 20 allows deep n_pt to reach PT817 feature depth
...
# LOGGING
# ==========================================
LOG_FILE_PATH = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_log.md"
...
# ==========================================
print("=" * 70)
print("7_mar_SC3_rad_PT768xCT8_325_deepPT_pc")
...
for method in RANKING_METHODS:
    ...
    for gm_name in GM_MODEL_NAMES:
        ...
        # Deep-PT seed enqueues targeting PT817 feature neighbourhood.
        # PT817 in PT768 rankings: GBS rank 1,2,4,6,9,12,16,18,34 -> n_pt>=16 covers 7/9
        # UNIVAR rank 1,2,5,8,11,19,30,31,32 -> n_pt>=19 covers 7/9
        # StabLASSO rank 5,6,14,20,22,27,29,30,34 -> n_pt>=30 covers 8/9
        # OOF(PT817 feats, dev) = 0.719 CoxPH/SVM -- 0.057 above core-6 attractor (0.662)
        # Dense seeding: exhaustive design means TPE samples around every seed heavily
        if method == "GBS":
            if gm_name == "SVM":
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 12, "clin_ratio": 0.08, "n_pt": 10, "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 8,  "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_svm": 1e-3})
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_svm": 1e-4})
            elif gm_name == "CoxPH":
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 12, "clin_ratio": 0.08, "n_pt": 10, "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 8,  "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_cox": 1.0})
        elif method == "UNIVAR":
            if gm_name == "SVM":
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 18, "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.07, "n_pt": 13, "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 11, "clin_ratio": 0.09, "n_pt": 9,  "alpha_svm": 0.01})
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 18, "alpha_svm": 1e-3})
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 18, "alpha_svm": 1e-4})
            elif gm_name == "CoxPH":
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 18, "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 15, "clin_ratio": 0.07, "n_pt": 13, "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 11, "clin_ratio": 0.09, "n_pt": 9,  "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 20, "clin_ratio": 0.05, "n_pt": 18, "alpha_cox": 1.0})
            ...
            if gm_name == "SVM":
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_svm": 1e-3})
                study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 8,  "alpha_svm": 5e-4})
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_svm": 0.01})
            elif gm_name == "CoxPH":
                study.enqueue_trial({"total_n": 18, "clin_ratio": 0.06, "n_pt": 16, "alpha_cox": 0.1})
                study.enqueue_trial({"total_n": 10, "clin_ratio": 0.10, "n_pt": 8,  "alpha_cox": 0.1})
        ...
        ckpt_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_checkpoint.csv"
...
out_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_all_results.csv"
...
top20_ext_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_top20_ext.csv"
...
top20_trio_path = OUTPUT_DIR / "7_mar_SC3_rad_PT768xCT8_325_deepPT_pc_top20_trio.csv"
...
print("\n7_mar_SC3_rad_PT768xCT8_325_deepPT_pc finished.")
